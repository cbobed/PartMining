...
#     return centroids
    # more pythonic way
    return { label: np.mean([model[it] for it in  labelled_transactions[label]], axis=0) for label in labelled_transactions}

def calculate_normalized_centroids(model, labelled_transactions):
    dim = model[labelled_transactions[0][0]].shape[0]
    return { label: preprocessing.normalize(np.mean([model[it] for it in labelled_transactions[label]], axis=0).reshape(1,dim), norm='l2').reshape(dim,)
...
def calculate_weighted_centroids(model, labelled_transactions):
    ...
    weights = {it: model.get_vecattr(it, "count") / len(labelled_transactions) for it in model.key_to_index}
    for label in labelled_transactions:
        item_vect = []
        weight_vect = []
        for it in labelled_transactions[label]:
            item_vect.append(model.get_vector(it))
...
if __name__ == "__main__":
    ...
    labelled_vects = {int(x): model.get_vector(x) for x in model.key_to_index if x != '<unk>'}
