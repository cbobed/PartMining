...
if [[ $CLUSTERING != "random" ]]; then
        ...
        { time ./GloVe.sh $1 >> "$OUTPUT_FILE" 2>>"$ERR_FILE" ; } 2>> "$TIME_FILE"
fi

echo "converting the glove vectors ..."
python sid/PartMining/gloveConverter.py $1.gloveVect.txt
echo "done ... "

# we should now have database_name+'_DIMENSION_WIN_EPOCHS_sg.vect' as name of the model
MODEL_FILE=$1.w2v.vect
echo $MODEL_FILE

echo "split database ... " >> "$TIME_FILE"
 echo $1 $MODEL_FILE $CLUSTERING $GRANULARITY $NUM_CLUSTERS $NORMALIZE
{ time ./splitDatabaseGlove.sh $1 $MODEL_FILE $CLUSTERING $GRANULARITY $NUM_CLUSTERS $NORMALIZE >> "$OUTPUT_FILE" 2>>"$ERR_FILE" ; } 2>>"$TIME_FILE"
